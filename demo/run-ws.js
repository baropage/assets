...
$(document).ready( function() {
	cf.frameLeft=$('.panel-left').find('iframe');
	cf.frameRight=$('.panel-right').find('iframe');
	cf.frameLeft.attr('src', "editor.bsp?code=_runScript" );
	setTimeout(setEditorConfig, 1500);
	resizePage();
});
...
function resizePage() {
  contentResize('.panel-container','#page-footer');
...
function setEditorConfig() {
	...
	if( editorConf && editorConf.editor ) {
    	...
    	cf.frameRight.attr('src', "editor.bsp?code=_log" );
    	setTimeout(setLogConfig, 1500);
		return;
	}
	setTimeout(setEditorConfig, 500);
}
$('.btn-my.run').on('click',function(e) {
	e.preventDefault();
	runSource();
});
$('.btn-my.clearLog').on('click',function(e) {
